<?php
/**
 * LunaticChO 管理后台 - 完整版（含联考出题、时间控制、收卷）
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_exam') {
        ...
        $stmt = $pdo->prepare("INSERT INTO gsk_exams (title, description, teacher, type, status, published_at, start_time, end_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $published_at = $status === 'published' ? date('Y-m-d H:i:s') : null;
        $stmt->execute([$title, $description, $teacher, $type, $status, $published_at, $start_time, $end_time]);
    ...
    if ($action === 'update_exam') {
        ...
        $stmt = $pdo->prepare("UPDATE gsk_exams SET title=?, description=?, type=?, status=?, start_time=?, end_time=? WHERE id=?");
        $stmt->execute([$title, $description, $type, $status, $start_time, $end_time, $exam_id]);
        $msg = '考试信息更新成功！';
    ...
    if ($action === 'end_exam') {
        $exam_id = $_POST['exam_id'];
        $stmt = $pdo->prepare("UPDATE gsk_exams SET status = 'ended', end_time = NOW() WHERE id = ?");
        $stmt->execute([$exam_id]);
        $msg = '考试已结束（收卷）！';
        header('Location: admin.php?exam_id=' . $exam_id . '&msg=' . urlencode($msg));
        exit;
...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .badge-ended { background: #fce4ec; color: #b91c1c; }
...
<div class="container">
        ...
        <div class="exam-list">
            <?php foreach ($exams as $e): ?>
                <div class="exam-card" onclick="window.location.href='?exam_id=<?= $e['id'] ?>'">
                    ...
                    <div class="meta">
                        <span>👩‍🏫 <?= htmlspecialchars($e['teacher']) ?></span>
                        <span>📅 <?= date('Y-m-d', strtotime($e['created_at'])) ?></span>
                    </div>
                    <div class="meta">
                        <span>类型: <?= $e['type'] === 'exam' ? '联考' : '周常' ?></span>
                        <span>状态: <span class="badge badge-<?= $e['status'] ?>">
                            <?= ['draft'=>'草稿','published'=>'已发布','grading'=>'批改中','completed'=>'已完成','ended'=>'已结束'][$e['status']] ?? $e['status'] ?>
                        </span></span>
                    </div>
                    <div class="status" style="margin-top:0.5rem;">
                        ...
                        <?php endif; ?>
                        <?php if ($e['type'] === 'exam' && $e['status'] !== 'ended' && $e['status'] !== 'draft'): ?>
                            <form method="POST" class="inline" onsubmit="return confirm('确认结束此考试（收卷）？')">
                                <input type="hidden" name="action" value="end_exam">
                                <input type="hidden" name="exam_id" value="<?= $e['id'] ?>">
                                <button type="submit" class="btn btn-warning btn-sm">收卷</button>
                            </form>
                        <?php endif; ?>
                        <?php if ($e['status'] !== 'draft'): ?>
    ...
    <div class="section">
        ...
        <div style="margin:0.5rem 0; display:flex; flex-wrap:wrap; gap:0.5rem;">
            <span>类型: <?= $exam['type'] === 'exam' ? '联考' : '周常' ?></span>
            <span>状态: <span class="badge badge-<?= $exam['status'] ?>">
                <?= ['draft'=>'草稿','published'=>'已发布','grading'=>'批改中','completed'=>'已完成','ended'=>'已结束'][$exam['status']] ?? $exam['status'] ?>
            </span></span>
            <?php if ($exam['start_time']): ?>
                <span>开始: <?= date('Y-m-d H:i', strtotime($exam['start_time'])) ?></span>
            <?php endif; ?>
            <?php if ($exam['end_time']): ?>
                <span>结束: <?= date('Y-m-d H:i', strtotime($exam['end_time'])) ?></span>
            <?php endif; ?>
        </div>
        ...
        <?php endif; ?>
        <?php if ($exam['type'] === 'exam' && $exam['status'] !== 'ended' && $exam['status'] !== 'draft'): ?>
            <form method="POST" class="inline" onsubmit="return confirm('确认结束此考试（收卷）？')">
                <input type="hidden" name="action" value="end_exam">
                <input type="hidden" name="exam_id" value="<?= $exam['id'] ?>">
                <button type="submit" class="btn btn-warning btn-sm">收卷</button>
        ...
        <details>
            <summary style="cursor:pointer; font-weight:600; color:#0b3b4c;">✏️ 编辑考试信息</summary>
            <form method="POST" style="margin-top:1rem;">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group" style="flex:1;">
                        <label>类型</label>
                        <select name="type">
                            <option value="weekly" <?= $exam['type']==='weekly'?'selected':'' ?>>周常</option>
                            <option value="exam" <?= $exam['type']==='exam'?'selected':'' ?>>联考</option>
                        </select>
                    </div>
                    <div class="form-group" style="flex:1;">
                        <label>状态</label>
                        <select name="status">
                            <option value="draft" <?= $exam['status']==='draft'?'selected':'' ?>>草稿</option>
                            <option value="published" <?= $exam['status']==='published'?'selected':'' ?>>已发布</option>
                            <option value="ended" <?= $exam['status']==='ended'?'selected':'' ?>>已结束</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    <label>说明</label>
                    <textarea name="description" rows="2"><?= htmlspecialchars($exam['description'] ?? '') ?></textarea>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label>开始时间</label>
                        <input type="datetime-local" name="start_time" value="<?= $exam['start_time'] ? date('Y-m-d\TH:i', strtotime($exam['start_time'])) : '' ?>">
                    </div>
                    <div class="form-group">
                        <label>结束时间</label>
                        <input type="datetime-local" name="end_time" value="<?= $exam['end_time'] ? date('Y-m-d\TH:i', strtotime($exam['end_time'])) : '' ?>">
                    </div>
                </div>
